...
const DOC_TYPES = ['', 'BANK_STATEMENT', 'TAX_RETURN', 'PURCHASE_CONTRACT', 'APPRAISAL', 'TITLE_REPORT', 'INSURANCE', 'ID', 'OTHER'];
...
export default function BrokerDocumentsPage() {
  ...
  const { data: documents = [], isLoading, isError } = useQuery({
    queryKey: ['broker-documents'],
    queryFn: () => api.get('/api/documents').then((r) => r.data),
  });

  const filtered = documents.filter((doc: any) => {
    const q = search.toLowerCase();
    const matchSearch = !q || (doc.name || '').toLowerCase().includes(q) || (doc.loanNumber || '').toLowerCase().includes(q);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-gray-500 text-sm">{documents.length} total documents across all loans</p>
      </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      {doc.url ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 px-3 py-1.5 rounded-lg"
                        >
